...
from .config import LLMWorkerConfig, FSDPConfig
...
class FSDPLLMWorker(LLMWorker):
    ...
    def init_model(self):
        ...
        if torch_dtype is None:
            torch_dtype = torch.bfloat16 if self.freeze else torch.float32
        ...
        if self.model_config.use_remove_padding:
            from verl.models.registry import check_model_support_rmpad
            check_model_support_rmpad(self.hf_model_config.model_type)

        if self.model_config.use_remove_padding and self.ulysses_sequence_parallel_size > 1:
            from verl.models.transformers.monkey_patch import apply_monkey_patch
            apply_monkey_patch(self.hf_model_config, verbose=True)
        ...
        with init_context(), warnings.catch_warnings():
            ...
            # TODO: add AutoModelForTokenClassification
